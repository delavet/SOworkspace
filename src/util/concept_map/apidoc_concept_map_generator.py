...
def __add_package_node_javadoc(concept_map : nx.DiGraph):
    ...
    for node in concept_map.nodes:
        if Ntype_attributes[node] == NodeType.PACKAGE:
            return
    module_nodes = [node for node in concept_map.nodes if Ntype_attributes[node] == NodeType.MODULE]
    for node in module_nodes:
        local_href = concept_map.nodes[node]['local_href']
        browser.get(local_href)
        time.sleep(0.05)
        packages = []
        block_sections = browser.find_elements(By.XPATH, r'//li[@class="blockList"]/section')
        for block_section in block_sections:
            block_title = block_section.find_element(By.XPATH, './h2')
            if block_title.text != 'Packages':
                continue
            package_groups = block_section.find_elements(By.XPATH, './div/table')
            for package_group in package_groups:
                relation_name = package_group.find_element(By.XPATH, './caption/span[1]').text
...
def __add_class_level_node_javadoc(concept_map : nx.DiGraph):
    ...
    global browser
    Ntype_attributes = nx.get_node_attributes(concept_map, 'Ntype')
    package_nodes = [node for node in concept_map.nodes if node in Ntype_attributes and Ntype_attributes[node] == NodeType.PACKAGE]
    for package_node in package_nodes:
        ...
        for block in blocks:
            ...
            if content_table is None:
                continue
            block_title = content_table.find_element(By.XPATH, r'./caption').text.lower()
            temps = []
...
def __add_member_node_to_class_level_node_javadoc(concept_map : nx.DiGraph, node_type : str):
    ...
    global browser
    Ntype_attributes = nx.get_node_attributes(concept_map, 'Ntype')
    class_nodes = [node for node in concept_map.nodes if node in Ntype_attributes and Ntype_attributes[node] == node_type]
    for class_node in class_nodes:
        try:
            ...
            try:
                summary_section = content_container.find_element(By.XPATH, r'.//section[@class="summary"]')
                blocks = summary_section.find_elements(By.XPATH, r'.//li[@class="blockList"]')
            except Exception as e:
                print('node ', class_node, ': nothing found')
                continue
            for block in blocks:
                block_title = block.find_element(By.XPATH, r'.//h2').text.lower()
                try:
                    member_summary_div = block.find_element(By.XPATH, r'.//div[@class="memberSummary"]')
                except NoSuchElementException as no_element:
                    continue
                member_summary_table = member_summary_div.find_element(By.XPATH, r'.//table')
                trs = member_summary_table.find_elements(By.XPATH, r'./tbody/tr')
                temps = []
                for tr in trs:
                    th_span = tr.find_element(By.XPATH, r'.//span[@class="memberNameLink"]')
...
def __add_member_node_to_class_level_nodes_javadoc(concept_map : nx.DiGraph):
    ...
    __add_member_node_to_class_level_node_javadoc(concept_map, NodeType.ANNOTATION)
    ...
    __add_member_node_to_class_level_node_javadoc(concept_map, NodeType.EXCEPTION)
    __add_member_node_to_class_level_node_javadoc(concept_map, NodeType.INTERFACE)
...
def __add_Parameter_ReturnType_relation(concept_map : nx.DiGraph):
    ...
    for node_type in class_level_node_types:
        ...
        for class_node in class_nodes:
            ...
            method_exist = True
            try:
                method_detail_section = content_container.find_element(By.XPATH, r'.//section[@class="methodDetails"]')
                method_blocks = method_detail_section.find_elements(By.XPATH, r'.//li[@class="blockList"]')
            except:
                method_exist = False
            if method_exist:
                for method_block in method_blocks:
                    try:
                        method_detail = method_block.find_element(By.XPATH, r'./section[@class="detail"]')
                        a_with_id = method_detail.find_element(By.XPATH, r'./h3/a[last()]')
                        method_id = a_with_id.get_attribute("id")
                        method_node = class_node + '#' + method_id
                        method_signature = method_detail.find_element(By.XPATH, r'.//div[@class="memberSignature"]')
                        try:
                            return_type = method_signature.find_element(By.XPATH, r'./span[@class="returnType"]/a')
                            return_type_node = get_relative_path_from_href(return_type.get_attribute('href'))
                            concept_map.add_edge(
                                method_node,
                                return_type_node,
                            ...
                            pass
                        try:
                            parameters = method_signature.find_elements(By.XPATH,r'./span[@class="arguments"]/a')
                            for parameter in parameters:
                                parameter_node = get_relative_path_from_href(parameter.get_attribute('href'))
                                concept_map.add_edge(method_node,parameter_node,Etype = EdgeType.PARAMETER)
                        except NoSuchElementException:
                            pass
                    except:
                        continue
            ...
            try:
                constructor_detail_section = content_container.find_element(By.XPATH, r'.//section[@class="constructorDetails"]')
                constructor_blocks = constructor_detail_section.find_elements(By.XPATH, r'.//li[@class="blockList"]')
            except:
            ...
            field_exist = True
            try:
                field_detail_section = content_container.find_element(By.XPATH, r'.//section[@class="fieldDetails"]')
                field_blocks = field_detail_section.find_elements(By.XPATH, r'.//li[@class="blockList"]')
            except:
                field_exist = False
            if field_exist:
                for field_block in field_blocks:
                    try:
                        field_detail = field_block.find_element(By.XPATH, r'./section[@class="detail"]')
                        a_with_id = field_detail.find_element(By.XPATH, r'./h3/a[last()]')
                        field_id = a_with_id.get_attribute("id")
                        field_node = class_node + '#' + field_id
                        field_signature = field_detail.find_element(By.XPATH, r'.//div[@class="memberSignature"]')
                        try:
                            return_type = field_signature.find_element(By.XPATH, r'./span[@class="returnType"]/a')
                            return_type_node = get_relative_path_from_href(return_type.get_attribute('href'))
                            concept_map.add_edge(
                                field_node,
                                return_type_node,
...
def __add_ref_in_desc_relation_javadoc(concept_map : nx.DiGraph):
    ...
    for node_type in class_level_node_types:
        ...
        for class_node in class_nodes:
            ...
            try:
                description_block = content_container.find_element(By.XPATH, r'.//section[@class="description"]/div')
            ...
            try:
                inheritance_tree = content_container.find_element(By.XPATH, r'./div[@class="inheritance"]')
                parent = inheritance_tree.find_element(By.XPATH, r'./a')
                child = inheritance_tree.find_element(By.XPATH, r'./div[@class="inheritance"]')
                terminited = False
                while not terminited:
                    try:
                        new_parent = child.find_element(By.XPATH, r'./a')
                        parent = new_parent
                        child = child.find_element(By.XPATH, r'./div[@class="inheritance"]')
                    except NoSuchElementException:
                        terminited = True
            ...
            try:
                description_block = content_container.find_element(By.XPATH, r'.//section[@class="description"]')
                    ...
                    try:
                        ...
                        for i in range(len(dts)):
                            ...
                            if 'Implemented' in dt.text:
                                implement_as = dd.find_elements(By.XPATH, r'.//a')
                                for implement_a in implement_as:
                                    implement_href = implement_a.get_attribute("href")
                                    relative_path = get_relative_path_from_href(implement_href)
                                    concept_map.add_edge(
                                        class_node,
                                        relative_path,
                            ...
                            if 'See' in dt.text or 'Note' in dt.text:
                                also_see_as = dd.find_elements(By.XPATH, r'.//a')
                                for also_see_a in also_see_as:
                                    also_see_href = also_see_a.get_attribute("href")
                                    relative_path = get_relative_path_from_href(also_see_href)
...
def generate_basic_concept_map_javadoc(mode = 'new_generate'):
    ...
    '''
    graph_path = os.path.join(CONCEPT_MAP_STORE_PATH, JAVADOC_CONCEPT_MAP_FILE_NAME)
