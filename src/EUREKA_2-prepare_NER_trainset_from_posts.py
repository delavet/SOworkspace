...
def _generate_train_data(sentence: str, target_doc: str = JAVADOC_GLOBAL_NAME):
    ...
    stop_words = ['here', 'this', 'javadoc', 'javadocs', 'docs', 'documentation', 'the documentation',
    ...
    try:
        ...
        # 筛选包含目标文档链接的超链接
        # 指向api的才要、太短的句子不要。。。
        a_texts = [a.text for a in soup.find_all('a') if len(a.text) > 1 and re.search(
            APIDOC_API_URL_REGEX_PATTERN[JAVADOC_GLOBAL_NAME], a.get('href')) is not None and a.text.lower() not in stop_words]
        if len(a_texts) <= 0:
            return ret
        # 原来这里直接按行切分，现在按照句号问号冒号叹号分割，试图让分割出来的句子短一些
        #lines = soup.text.split('\n')
        lines = re.split(r'(\s*(\.|:|\?|!|\~)\s+|\n)+', soup.text)
        candidate_lines = [line for line in lines if any(
            [t for t in a_texts if t in line]) and len(line.split(' ')) > 2]
